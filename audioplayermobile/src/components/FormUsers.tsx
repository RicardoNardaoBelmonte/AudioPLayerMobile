...
const FormUsers: React.FC<Omit<IFormUsuarios, 'setNome'>> = ({ setVisibleFormUsuarios, tipoFormUsuarios, setIsLogged }) => {
  ...
  const loginMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setToken(data.token);   // atualiza o estado global
  ...
  const registerMutation = useMutation({
    ...
    onError: (e: any) => alert(e.message || "Erro ao fazer registro"),
  });

  const [formData, setFormData] = useState<{ username: string; password: string }>({ username: "", password: "" });

  function handleSubmit(e: React.FormEvent<HTMLFormElement>, data: { username: string; password: string }, type: string) {
    e.preventDefault();
    if (type === "Logar") {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 items-center flex justify-center z-50">
      <div className="flex flex-col bg-background border border-borderGray p-5 w-72 h-72 rounded-lg relative">
        <IonButton aria-label="Fechar" className="text-gray-400 absolute right-1 top-2" onClick={() => setVisibleFormUsuarios(false)}>X</IonButton>

        <form className="flex flex-col justify-center items-center gap-3" onSubmit={(e) => handleSubmit(e, formData, tipoFormUsuarios)}>
          <h2 className="text-primary">{tipoFormUsuarios}</h2>

          <IonButtons class="flex flex-col gap-5 justify-center items-center">
            <IonInput
              name="username"
              type="text"
              placeholder='Nome'
              className='!pl-1 border border-borderGray rounded !text-white focus:outline-borderGray'
            ...
            <IonButton type='submit' className='formUsuarioBtn capitalize bg-primary cursor-pointer text-black !rounded w-20 h-10' aria-label='Enviar'>
              Enviar
            </IonButton>
          </IonButtons>
        </form>
